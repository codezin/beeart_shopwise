...
namespace Theme\JangSeang1\Http\Controllers;
...
use Theme;
use Theme\AnonEcommerce\Http\Resources\BrandResource;
use Theme\AnonEcommerce\Http\Resources\PostResource;
use Theme\AnonEcommerce\Http\Resources\ProductCategoryResource;
use Theme\AnonEcommerce\Http\Resources\ReviewResource;
use Theme\AnonEcommerce\Http\Resources\TestimonialResource;
use DB;
...
class ShopwiseController extends PublicController
{
    public function ajaxGetProducts(Request $request, BaseHttpResponse $response)
    {
        ...
        foreach ($products as $product) {
            ...
            if(request('lang')!='ko_KR')
            {
                $condition = [
                    'lang_code' => request('lang'),
                    'ec_products_id' => $product->id,
                ];
                $existing = DB::table('ec_products_translations')->where($condition)->first();

                if(!empty($existing))
                {
                    $product->name =    $existing->name;
                    $product->description =  $existing->description;
                    $product->content =  $existing->content;
                }
            }
