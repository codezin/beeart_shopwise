            ...
            @foreach (Cart::instance('cart')->content() as $key => $cartItem)
                ...
                @if (!empty($product))
                    <li>
                        ...
                        <a href="{{ $product->original_product->url }}"><img src="{{ RvMedia::getImageUrl($cartItem->options['image'], 'thumb', false, RvMedia::getDefaultImage()) }}" alt="{{ $product->name }}" loading="lazy" /> {{ $product->name }} @if ($product->isOutOfStock())
