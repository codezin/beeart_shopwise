<ul {!! $options !!}>
    ...
    <li @if ($row->has_child || $row->css_class || $row->active) class="@if ($row->has_child) dropdown has-dropdown @endif @if ($row->css_class) {{ $row->css_class }} @endif " @endif>
        <a class="@if ($row->has_child) nav-link @else nav-link nav_item @endif @if ($row->active || in_array("/".request()->segment(1),array_column($row->child->toArray(),"url")) ) active @endif" href="{{ $row->has_child ? $row->url : url($row->url) }}" @if ($row->target !== '_self') target="{{ $row->target }}" @endif @if ($row->has_child) data-toggle="dropdown" @endif>
